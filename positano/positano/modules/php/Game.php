...
//       }
    ...
    protected function zombieTurn(array $state, int $active_player): void
    {
        ...
        $this->notify->all('zombie', clienttranslate('Entering zombie turn for state ' . $state_name . ' and player ' . (string)$active_player));
        ...
        switch ($state_name) {
            case 'playerBid':
                ...
                break;
            case 'playerPickLot':
                $this->zombiePickLot($active_player);
                break; 
            case 'playerPickBlocks':
                $this->zombiePickBlocks($active_player);
                break; 
            case 'playerPickQuality':
                $this->zombiePickQuality($active_player);
                break; 
            case 'playerTurn':
                $this->zombieBuild($active_player);
                break; 
            default:
                //$this->gamestate->nextState("zombiePass");
                $this->zombieRennovate($active_player);
                break;
    ...
    public function zombieBid(int $player_id): void {
        $this->cards->shuffle($player_id);
        $zombie_cards = $this->cards->getCardsInLocation( $player_id );
        ...
        $this->actPlayBid($bid_id, $booster_id);
    }

    public function zombiePickLot(int $player_id): void {
        $zombie_cards = $this->cards->getCardsInLocation( 'lot_stock' );

        $this->notify->all('zombie', clienttranslate('cards in lot stock: ' . (string)count($zombie_cards)));
        $rand_id = bga_rand(0, count($zombie_cards) - 1);
        $i = 0;
        $chosen_lot = 0;
        foreach($zombie_cards as $card){
            if ($i == $rand_id) {
                $chosen_lot = (int)$card['type_arg'];
            }
            $i ++;
        }

        /*$chosen_lot = (int)$zombie_cards[bga_rand(0, count($zombie_cards) - 1)]['type_arg'];

        $this->notify->all('zombie', clienttranslate((string)$chosen_lot));
        */
        //$chosen_lot = 20;
        $this->actPickLot($chosen_lot);
    }

    public function zombiePickBlocks(int $player_id): void {
        $zombie_cards = $this->cards->getCardsInLocation( 'block_stock' );
        //$id = (int)$zombie_cards[bga_rand(0, count($zombie_cards) - 1)]['type_arg'];
        ...
        $block_name = $this->get_blocks($id)[0];
        $block_count = (int)$this->getAllDatas()['players'][$player_id]['blocks'];
        $selection = 1;
        ...
        $this->actPickBlocks($id, $selection);
    }

    public function zombiePickQuality(int $player_id): void {
        $zombie_cards = $this->cards->getCardsInLocation( 'quality_stock' );
        //$id = (int)$zombie_cards[bga_rand(0, count($zombie_cards) - 1)]['type_arg'];
        //if (count($zombie_cards) == 0){
        //$rand_id = 0;
        $this->notify->all('zombie', clienttranslate('cards in quality stock: ' . (string)count($zombie_cards)));
        //} else {
        $rand_id = bga_rand(0, count($zombie_cards) - 1);
        ...
        $this->actPickQuality($id);
    }

    public function zombieBuild(int $player_id): void {
        $block_count = (int)$this->getAllDatas()['players'][$player_id]['blocks'];
        $lot_id = (int)$this->getObjectListFromDB("SELECT `lot_pick` FROM `player` WHERE `player_id` = '$player_id'", true)[0];
        $quality_name = $this->getObjectFromDb("SELECT `quality_pick` `q` FROM `player` WHERE `player_id` = '$player_id'")['q'];
        $lot_name = self::$LOT_TYPES[$lot_id]['card_name'];
        $max_height = 0;
        $y = (int)substr($lot_name, 0, 1);
        $height_extension = 0;
        if (count(explode('Build', $quality_name)) > 1){
            $height_extension = (int)explode(' ', explode('+', $quality_name)[1])[0];
        }
        $max_height = $y + $height_extension;

        if ($block_count < $max_height){
            $max_height = $block_count;
        }
        ...
        $this->actBuild($lot_id, $max_height);
    }

    public function zombieRennovate(int $player_id): void { //TODO make zombie rennovate random building
        $player_name = $this->getPlayerNameById($player_id);
        $quality_id = (int)$this->getObjectListFromDB("SELECT `quality_id` FROM `player` WHERE `player_id` = '$player_id'", true)[0];
        $this->notify->all("pass", clienttranslate('${player_name} passes'), [
            "player_id" => $player_id,
            "player_name" => $player_name,
            "quality_id" => $quality_id,
